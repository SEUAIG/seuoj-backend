# 测试用例目录
本目录用于记录集成测试的用例名称、场景与预期，确保每个测试都有明确规范，避免无意义测试。
...
| INT-AUTH-004 | `TokenExchangeIntegrationTest#exchangeTempTokenShouldReturnAccessToken` | 使用短期令牌兑换长期访问令牌 | 200 | 0 | 返回 `access_token`，且不返回 `temp_token` |
| INT-AUTH-005 | `TokenExchangeIntegrationTest#exchangeAccessTokenShouldReturnTempToken` | 使用长期访问令牌兑换短期令牌 | 200 | 0 | 返回 `temp_token`，且不返回 `access_token` |
| INT-AUTH-006 | `TokenExchangeIntegrationTest#exchangeInvalidTokenShouldReturn401` | 使用非法令牌调用交换接口 | 401 | 40100 | 返回未登录错误 |
| INT-AUTH-007 | `TokenExchangeIntegrationTest#userMeShouldReturnProfileWhenAccessTokenValid` | 使用访问令牌获取当前用户信息 | 200 | 0 | 返回 `uuid/username/email` |
| INT-AUTH-008 | `TokenExchangeIntegrationTest#userMeShouldReturn401WhenTempTokenUsed` | 使用短期令牌访问用户信息接口 | 401 | 40100 | 返回未登录错误 |
