...
@Component
public class OiRankingStrategy implements ContestRankingStrategy {
    ...
    public List<ContestStandingsRecordVO> computeStandings(
        ...
        // For each (user, problem): take the LAST submission's score
        Map<Long, Map<Long, ProblemScore>> userMap = new HashMap<>();
        for (Long userId : registeredUsers.keySet()) {
            userMap.put(userId, new HashMap<>());
        }

        // finishedSubmissions is ordered by submit_time ASC, so last write wins
        for (Submission s : finishedSubmissions) {
            ...
            ps.score = s.getScore() != null ? s.getScore() : 0;
